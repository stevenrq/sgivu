...
### `DEV_REDIS_PASSWORD` / `PROD_REDIS_PASSWORD`

Se usa en **tres lugares distintos** que deben coincidir:

1. Comando `--requirepass` del contenedor `sgivu-redis`.
2. `spring.data.redis.password` en `sgivu-gateway` (para sesiones HTTP del BFF).
3. `spring.data.redis.password` en `sgivu-purchase-sale` (para el caché del dashboard).

Si difieren, el gateway no puede conectarse a Redis → todas las sesiones HTTP fallan → no hay autenticación; o purchase-sale no puede cachear y degrada el dashboard.

Redis lo usan dos servicios con namespaces independientes: `sgivu-gateway` (sesiones HTTP, namespace `spring:session:sgivu-gateway`) y `sgivu-purchase-sale` (caché del dashboard, namespace `sgivu:cache:purchase-sale:`, TTL 60 s).
...
| Variable                                     | Consumida por                                            | Por qué existe                      |
| -------------------------------------------- | -------------------------------------------------------- | ----------------------------------- |
| `DEV_REDIS_HOST` / `PROD_REDIS_HOST`         | `sgivu-gateway`, `sgivu-purchase-sale`                   | Host del contenedor Redis.          |
| `DEV_REDIS_PORT` / `PROD_REDIS_PORT`         | `sgivu-gateway`, `sgivu-purchase-sale`                   | Puerto de Redis.                    |
| `DEV_REDIS_PASSWORD` / `PROD_REDIS_PASSWORD` | `sgivu-gateway`, `sgivu-purchase-sale`, `sgivu-redis`    | Ver sección de dependencias arriba. |
...
> **Creación automática de bases de datos:** Al primer inicio (volumen `postgres-data` vacío), el script `postgres-init/create-databases.sh` crea las 6 bases de datos: `sgivu_auth_db`, `sgivu_user_db`, `sgivu_client_db`, `sgivu_vehicle_db`, `sgivu_purchase_sale_db` y `sgivu_ml_db`. Las tablas y datos semilla los gestionan Flyway (Spring Boot) y Alembic (`sgivu-ml`). Si el volumen ya tiene datos, el script no se re-ejecuta (comportamiento estándar de `docker-entrypoint-initdb.d`).
...
| `DATABASE_RUN_MIGRATIONS`            | `true` ejecuta migraciones Alembic al arrancar (`alembic upgrade head`).                                                                    |
