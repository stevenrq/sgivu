...
> **Inicialización automática de Zipkin:** La imagen `mysql:8` crea automáticamente la base de datos (`MYSQL_DATABASE`), el usuario (`MYSQL_USER`) y le asigna permisos usando las variables del `.env`. El script `mysql-init/00-configure-auth.sh` cambia el plugin de autenticación del usuario a `mysql_native_password` (requerido por el cliente MariaDB JDBC de Zipkin, ya que MySQL 8.4+ usa `caching_sha2_password` por defecto). Luego, `mysql-init/zipkin-user.sql` crea las tablas (`zipkin_spans`, `zipkin_annotations`, `zipkin_dependencies`) y sus índices. Todo se ejecuta en el primer inicio (volumen `mysql-data` vacío).
