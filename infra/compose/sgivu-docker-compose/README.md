...
```bash
chmod +x run.sh
./scripts/run.sh --dev    # stack de desarrollo
./scripts/run.sh --prod   # stack productivo
```
...
./scripts/dev-up.sh                       # postgres, redis, config, discovery
./scripts/dev-up.sh --with sgivu-auth     # añade auth si NO lo estás editando

# 2a. Lanzar uno o varios servicios desde el directorio de compose
./scripts/host-run.sh sgivu-user                  # solo user
./scripts/host-run.sh sgivu-user sgivu-vehicle    # varios a la vez
./scripts/host-run.sh --all                       # todas las apps en host (auth, client,
                                          # user, vehicle, purchase-sale, gateway, ml)
./scripts/host-run.sh --status                    # ver qué hay corriendo
./scripts/host-run.sh --logs sgivu-user           # tail -f del log
./scripts/host-run.sh --stop                      # detener todos los host
./scripts/host-run.sh --stop sgivu-user           # detener uno
...
| Comando | Cuándo |
| --- | --- |
| `./scripts/dev-up.sh` | Día a día. Levanta infra; tú corres el servicio editado en host. |
| `./scripts/dev-up.sh --with <svc>` | Cuando necesitas un servicio extra dockerizado. |
| `./scripts/run.sh --dev` | Smoke test del stack completo en Docker (imágenes existentes). |
| `./scripts/run-service.sh --dev <svc>` | Recrear un contenedor sin reconstruir. |
| `./scripts/rebuild-service.sh --dev <svc>` | Reconstrucción explícita (mvn + docker build) de un servicio. Antes de PR. |
| `./scripts/rebuild-service.sh --dev <svc> --push` | Igual, y publica a Docker Hub. |
| `./scripts/build-and-push-images.sh` | Reconstruye y publica TODAS las imágenes. Releases. |
...
- `./scripts/rebuild-service.sh --dev <servicio>` — reconstruye un servicio puntual
  (mvn package + docker build) y recrea solo su contenedor.
- `./scripts/build-and-push-images.sh` — recorre todos los servicios, los compila,
...
- **OAuth2 issuer matching (importante):** `sgivu-auth` reporta su issuer
  ...
  todos en host (`./scripts/host-run.sh sgivu-auth sgivu-user ...`) o todos en
  Docker (`./scripts/dev-up.sh --with sgivu-auth --with sgivu-user ...`). Mezclar
  rompe el bootstrap del `JwtDecoder`.
- **Divergencia dev vs prod:** correr en host puede ocultar problemas de
  contenedor (timezone, locale, paths). Antes de cada PR: `./scripts/rebuild-service.sh
...
./scripts/run-service.sh --dev sgivu-auth sgivu-user

# Levantar servicios en modo producción (usa docker-compose.yml y .env)
./scripts/run-service.sh --prod sgivu-gateway sgivu-vehicle
```
...
* * * * * /usr/bin/flock -n /tmp/sgivu-dbs-backups.lock /home/steven/Documents/github/sgivu/infra/compose/sgivu-docker-compose/scripts/dbs-backups.sh --dev >> /home/steven/Backups/sgivu-dbs-backups/cron.log 2>&1
```
...
- `./scripts/build-and-push-images.sh` construye y publica todas las imágenes `stevenrq/*` recorriendo los servicios hermanos.
- Invoca `build-image.sh` cuando existe o compila con Maven en proyectos Java.
- `./scripts/rebuild-service.sh --dev sgivu-auth` reconstruye la imagen localmente y recrea solo ese contenedor en el stack. Añada `--push` para publicar en Docker Hub (`./scripts/rebuild-service.sh --dev --push sgivu-auth`).
...
El despliegue normal es `make deploy` (workflow `.github/workflows/deploy.yml`). Antes del primer uso:

1. **Clave SSH dedicada para el deploy** (no reutilizar `sgivu-ec2-keypair.pem`):

   ```bash
   ssh-keygen -t ed25519 -f ~/.ssh/sgivu-deploy -N "" -C "sgivu-deploy"
   # autorizarla en EC2:
   ssh -i sgivu-ec2-keypair.pem ubuntu@<EC2_HOST> \
     "echo '$(cat ~/.ssh/sgivu-deploy.pub)' >> ~/.ssh/authorized_keys"
   ```

2. **Secrets en GitHub** (requiere `gh auth login`):

   ```bash
   gh secret set DOCKERHUB_USERNAME --body "stevenrq"
   gh secret set DOCKERHUB_TOKEN            # access token de Docker Hub (no la contraseña)
   gh secret set EC2_HOST --body "ec2-XX-XX-XX-XX.compute-1.amazonaws.com"
   gh secret set EC2_USER --body "ubuntu"
   gh secret set EC2_SSH_KEY < ~/.ssh/sgivu-deploy
   base64 -w0 ../../../apps/backend/sgivu-auth/src/main/resources/keystore.jks | gh secret set AUTH_KEYSTORE_B64
   ```

3. **`.env` de producción en EC2**: debe existir en `/home/ubuntu/sgivu-docker-compose/.env` (el workflow solo actualiza la línea `TAG=`, nunca sincroniza `.env`).

Rollback: `make deploy TAG=<sha-corto-anterior>` reutiliza imágenes ya publicadas (sin build).
