...
sequenceDiagram
    ...
    ModelTrainer->>ModelTrainer: Evaluate RandomForest + XGBoost (RandomizedSearchCV)
    ModelTrainer-->>TrainingService: best pipeline + metrics + baselines
...
### Gestión de Modelos
- Versionamiento automático con timestamps UTC (`YYYYMMDDHHMMSS`)
- Seguimiento de métricas de rendimiento (RMSE, MAE, MAPE, **WAPE**, R², `residual_std`)
- Comparación entre Random Forest y XGBoost en cada entrenamiento
- Baselines naive (`naive_lag1_rmse`) para validar el valor agregado del modelo
- Persistencia dual: filesystem (`*.joblib` + `latest.json`) o PostgreSQL (`ml_model_artifacts.artifact BYTEA`)
- Reentrenamiento programado (cron interno `RETRAIN_CRON=0 3 1 * *`, día 1 de cada mes a las 3 AM UTC)
...
# Construir imagen + publicar en Docker Hub
./build-image.sh --push

# Ejecutar contenedor
docker run --env-file .env -p 8000:8000 stevenrq/sgivu-ml:0.1.0
```

```bash Production
# Mediante enrutamiento del gateway
...
El directorio `scripts/` contiene herramientas para probar el modelo localmente usando los microservicios reales vía HTTP, sin necesidad de datos sintéticos:

| Script | Propósito |
|--------|-----------|
| `scripts/train_from_services.py` | Lanza un reentrenamiento obteniendo contratos de `sgivu-purchase-sale` y vehículos de `sgivu-vehicle` |
| `scripts/predict_from_services.py` | Realiza predicciones contra el servicio ML local usando segmentos reales |
| `scripts/run_dev.sh` | Levanta el entorno de desarrollo con las variables de entorno necesarias |

```bash
# Entrenar con datos reales de los microservicios
cd apps/ml/sgivu-ml
bash scripts/run_dev.sh python scripts/train_from_services.py

# Predecir usando un segmento real
bash scripts/run_dev.sh python scripts/predict_from_services.py
```

<Note>
Los scripts requieren que `sgivu-purchase-sale`, `sgivu-vehicle` y el servidor de autenticación estén accesibles (stack local con Docker Compose o servicios en desarrollo). Configura las URLs en las variables `SGIVU_PURCHASE_SALE_URL` y `SGIVU_VEHICLE_URL`.
</Note>
...
5. **Características temporales**: mes, año, codificación cíclica (sin/cos) de mes y trimestre (`quarter`, `quarter_sin`, `quarter_cos`)
...
El entrenador (`ModelTrainer`) evalúa **dos algoritmos** y, si `enable_hyperparameter_tuning=true`, ejecuta `RandomizedSearchCV` con `TimeSeriesSplit(n_splits=3)`:
...
Como referencia, `_compute_baselines` calcula también un baseline naive (`naive_lag1_rmse`) sobre el conjunto de prueba para verificar que el modelo entrenado supera la heurística más simple.

### Selección de Modelo

El mejor candidato se selecciona mediante una **puntuación compuesta ponderada** sobre el conjunto de prueba: `(1 - mape_weight) * RMSE + mape_weight * WAPE` con `mape_weight = 0.4` (modo `weighted`, por defecto). El criterio es configurable vía `MODEL_SELECTION_METRIC` (`weighted` | `rmse` | `mape`). Todas las métricas (RMSE, MAE, MAPE, WAPE, R²) se registran para auditoría junto con baselines naive.
...
| `ml_drift_records` | Registros de predicción vs valor real para monitoreo de drift |
...
- **WAPE**: Weighted Absolute Percentage Error — más estable que MAPE en segmentos con baja demanda; métrica usada por defecto para selección
- **R²**: Coeficiente de determinación
- **residual_std**: Desviación estándar de residuos (para intervalos de confianza)
- **baselines**: Métricas de referencia naive (p.ej. `naive_lag1_rmse`)
