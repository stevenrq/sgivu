...
**Ejemplo**: `20260306143022` representa un modelo entrenado el 6 de marzo de 2026 a las 14:30:22 UTC.
...
# Example usage
version = generate_version()  # "20260306143022"
```
...
El Pipeline incluye:
- **Preprocesador**: `ColumnTransformer` con `OneHotEncoder` para categóricas y `StandardScaler` + `SimpleImputer` para numéricas
- **Modelo**: Mejor estimador entre Random Forest y XGBoost (seleccionado por score compuesto WAPE+RMSE)
...
# Fetch model history
versions = [
    {"version": "20260301093045", "rmse": 4.2, "r2": 0.82},
    {"version": "20260305141530", "rmse": 3.8, "r2": 0.85},
    {"version": "20260306143022", "rmse": 3.24, "r2": 0.89},
]
...
mv MODEL_DIR/20260306143022 MODEL_DIR/20260306143022.backup

# Predictions will now use 20260305141530
```
...
```bash
PREFERRED_MODEL_VERSION=20260305141530
```
...
alerts:
  - name: NoModelAvailable
    ...
    action: Activar reentrenamiento

  - name: ModelPerformanceDegraded
    condition: rmse > 5.0 OR r2 < 0.70
    action: Revisar calidad de datos y reentrenar

  - name: PredictionErrors
    condition: error_rate > 5%
    action: Verificar segmentos faltantes y problemas de datos
```
