...
```json
{
  ...
  "iss": "http://localhost:9000",
  "aud": "sgivu-gateway",
  "exp": 1712246400,  // 7 days
  "iat": 1709652600
}
```

<Warning>
El ID Token tiene un **TTL de 7 días** (coincide con el tiempo de vida del Refresh Token) porque se usa como `id_token_hint` durante el logout de OIDC. Spring Security nunca refresca el ID Token, por lo que debe permanecer válido durante toda la sesión.
</Warning>
...
| Refresh Token | 7 días | Sí (se emite un nuevo token al refrescar) |
| ID Token | 7 días | No (nunca se refresca) |
...
@PreAuthorize("hasAuthority('car:create') and hasRole('ADMIN')")
@PostMapping("/v1/cars")
...
1. Generar un nuevo par de claves RSA con un nuevo `kid` (e.g., `sgivu-jwt-key-2024`)
2. Agregar al keystore junto con la clave anterior
3. Actualizar `sgivu.jwt.key.alias` al nuevo `kid`
4. El servidor de autorización comienza a firmar con la nueva clave
5. La clave anterior permanece en el JWKS por 7 días (tiempo de vida máximo del refresh token)
6. Eliminar la clave anterior después de 7 días

**¿Por qué 7 días?** Los tokens firmados con la clave anterior siguen siendo válidos hasta expirar. Los microservicios necesitan la clave pública anterior para validarlos.

## Documentación Relacionada

- [OAuth2 y OIDC](/security/oauth2-oidc) - Emisión de tokens y flujos de autorización
- [Patrón BFF](/security/bff-pattern) - Cómo el gateway gestiona los tokens
- [Comunicación entre servicios](/security/service-communication) - Autenticación interna entre servicios (sin JWT)
