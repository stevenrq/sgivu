...
`sgivu-gateway` usa Redis para persistir las Sessions del patrón BFF (namespace `spring:session:sgivu-gateway`). El mismo Redis también lo usa `sgivu-purchase-sale` como caché de agregados del dashboard, pero con un namespace independiente (`sgivu:cache:purchase-sale:`) y TTL distinto, por lo que no hay solapamiento entre ambos usos.
...
spring:
  session:
    ...
    timeout: 7d  # Sliding timeout (resets on activity)
  data:
    redis:
...
El TTL de Redis es **deslizante**: se reinicia en cada petición al Gateway. Para que las sesiones no expiren mientras el usuario tiene la aplicación abierta, la SPA Angular envía pings periódicos a `/auth/session`:
  ...
  merge(of(undefined), visible$)
    .pipe(
      switchMap(() => {
        ...
        return timer(0, AuthService.SESSION_KEEPALIVE_INTERVAL_MS); // cada 20 min
      }),
      switchMap(() => {
...
**Comportamiento:**
- Ping inmediato al cargar la app (si el tab es visible y el usuario está autenticado)
- Ping cada 20 minutos mientras el tab permanezca visible
- Al ocultar el tab (cambiar de pestaña, minimizar), los pings se detienen
- Al volver al tab, se envía un ping inmediato y se retoma el intervalo
- Si el ping recibe `401`, limpia el estado de autenticación (el refresh token expiró o fue revocado)

<Info>
El intervalo de 20 minutos garantiza que una sesión Redis de 7 días nunca expire por inactividad mientras la aplicación esté abierta, ya que el TTL se reinicia en cada ping.
</Info>
...
sequenceDiagram
    ...
    Angular->>Gateway: GET /v1/cars (with SESSION cookie)
    Gateway->>Redis: Load session
    Redis-->>Gateway: OAuth2AuthorizedClient
    
    alt Access token valid
        Gateway->>Service: GET /v1/cars<br/>Authorization: Bearer <access_token>
        Service-->>Gateway: Car data
        Gateway-->>Angular: Car data
        ...
        Gateway->>Service: GET /v1/cars<br/>Authorization: Bearer <new_token>
        Service-->>Gateway: Car data
        Gateway-->>Angular: Car data
    end
```
  ...
  return authorizeRequest -> authorizedClientManager
    ...
    .onErrorResume(ClientAuthorizationException.class, ex -> {
      if (OAuth2ErrorCodes.INVALID_GRANT.equals(ex.getError().getErrorCode())) {
        // Invalidar la sesión Redis huérfana para forzar re-autenticación limpia
...
1. El Gateway invalida la sesión Redis para limpiar el estado huérfano
2. `authorizedClientManager.authorize()` retorna `Mono.empty()`
3. `/auth/session` retorna `401 Unauthorized`
4. El filtro Token Relay omite agregar el header `Authorization`
5. Los microservicios retornan `401` por token faltante
6. Angular detecta el `401` y redirige al inicio de sesión
...
```bash
# Redis (requerido) — perfil prod
PROD_REDIS_HOST=redis.internal.example.com
PROD_REDIS_PORT=6379
PROD_REDIS_PASSWORD=<strong-password>

# Timeout de sesión (opcional, por defecto 7d)
