...
<Steps>
  ...
  <Step title="Configurar Variables de Entorno">
    ...
    # Service URLs
    # Hostname nip.io: resoluble desde el navegador (sin editar hosts) y desde los
    # contenedores (alias de red en docker-compose.dev.yml). ISSUER_URL debe coincidir.
...
<Steps>
  ...
  <Step title="Verificar los Cambios">
    ...
    # Verificar logs del servicio para confirmación
    docker compose logs -f sgivu-auth
    ```
  </Step>
</Steps>
...
```bash
# Detener el contenedor Docker del servicio que estás desarrollando
docker compose stop sgivu-auth

# Ejecutar el servicio localmente con tu IDE o CLI
...
<Tabs>
  <Tab title="Verificar Config Server">
    ...
    # Verificar que el repositorio Git esté actualizado (si usa backend Git)
...
```bash
# Verificar que el servicio pueda alcanzar el Config Server
...
# View Redis keys
docker compose exec redis redis-cli -a "${DEV_REDIS_PASSWORD}" keys "spring:session:*"
```
