...
| Servicio | Puerto | Propósito | Almacenamiento |
|----------|--------|-----------|----------------|
| **sgivu-config** | 8888 | Spring Cloud Config Server (sirve la configuración de los demás servicios) | Git (`sgivu-config-repo`) |
| **sgivu-discovery** | 8761 | Registro de Servicios Eureka | Ninguna |
| **sgivu-auth** | 9000 | Servidor de Autorización OAuth2.1/OIDC, emisor JWT | PostgreSQL (`sgivu_auth_db`) |
| **sgivu-gateway** | 8080 | API Gateway + BFF | PostgreSQL no; Redis (sesiones) |
| **sgivu-user** | 8081 | Gestión de Usuarios y permisos | PostgreSQL (`sgivu_user_db`) |
| **sgivu-client** | 8082 | Personas y empresas | PostgreSQL (`sgivu_client_db`) |
| **sgivu-vehicle** | 8083 | Catálogo de Vehículos | PostgreSQL (`sgivu_vehicle_db`) + AWS S3 (imágenes) |
| **sgivu-purchase-sale** | 8084 | Contratos de compraventa | PostgreSQL (`sgivu_purchase_sale_db`) + Redis (caché del dashboard) |
| **sgivu-ml** | 8000 | API de predicción y reentrenamiento (FastAPI) | PostgreSQL (`sgivu_ml_db`) |
