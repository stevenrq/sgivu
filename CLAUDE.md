...
- **Desarrollo**: Windows con **WSL** (Linux). Todos los comandos del proyecto (`make`, Docker, `./mvnw`, `uv`) se ejecutan dentro de WSL.
- **Producción**: instancia única de **AWS EC2** con Docker Compose y Nginx en el host (ver sección "Despliegue (Producción)").
...
El punto de entrada único es el `Makefile` raíz (`make help` lista todos los targets). Delegan en los scripts de `infra/compose/sgivu-docker-compose/`.

### Desarrollo

```bash
make dev                          # infra en Docker + apps en host con hot-reload
make up                           # stack completo dockerizado (run.sh --dev)
...
```bash
make test [SERVICE=sgivu-user]    # tests (todos o uno)
make lint / make format           # Spotless (Java) + black/pylint (Python)
make build [SERVICE=...]          # mvn package
make image SERVICE=sgivu-user     # imagen Docker local (multi-stage, autocontenida)
make rebuild SERVICE=sgivu-auth   # reconstruir servicio en stack activo
```

Test individual:

```bash
cd apps/backend/<servicio> && ./mvnw -B -Dtest=<ClaseTest> test   # Java
cd apps/ml/sgivu-ml && .venv/bin/pytest tests/<archivo>.py -q     # Python
```

### Despliegue (GitHub Actions, manual)

```bash
make deploy [SERVICES=...] [TAG=...]   # dispara deploy.yml; TAG = rollback sin build
make deploy-watch                      # seguir el run en vivo
```

**Notas para agentes:** hay hooks de pre-commit (Spotless/black+pylint) que pueden reformatear archivos al commitear; si un commit falla por reformateo, hacer `git add -u` y reintentar. El `keystore.jks` de sgivu-auth está gitignorado (secreto); en CI/CD se restaura desde el secret `AUTH_KEYSTORE_B64`.
...
- **Angular 21** con standalone components, lazy-loaded routes y change detection **zoneless** (`OnPush` obligatorio)
- **Bootstrap 5.3** + CSS variables personalizadas para temas light/dark
- **Reactive Forms** con tipado fuerte (`FormGroup<FormControls>`)
- **Vitest + jsdom** para tests unitarios (sin navegador); **Playwright** para e2e con red simulada
...
Carpeta `shared/` contiene 15+ componentes reutilizables: `data-table`, `pager`, `page-header`, `kpi-card`, `form-shell`, `navbar`, `sidebar`, `loading-overlay`, `skeleton`, `filter-chip-group`, `quick-search-bar`, `list-toolbar`, entre otros.
...
`ThemeService` gestiona preferencia y tema activo (light/dark) vía CSS variables de Bootstrap 5.3. Fuente: Manrope, empaquetada desde npm (`@fontsource/manrope`), sin CDN.
...
- `describe()` e `it()` (Angular / Vitest): **ESPAÑOL**.
...
**Angular (Vitest)** — Archivos: `<archivo>.spec.ts`. Usar `describe` anidados por método, `it` como oraciones:
...
- **Nunca commitear directo a `main`**: siempre crear rama y abrir PR.
- Commits en **inglés**, siguiendo **Conventional Commits** y **Gitflow** (los commits en español del historial fueron una excepción, no la regla).
- Commits pequeños y atómicos.
- Ramas con nombres claros (`main`, `develop`, `feature/`, `release/`, `hotfix/`, `fix/`, `refactor/`, `chore/`).
- El commit de formato masivo está en `.git-blame-ignore-revs`; usar `git blame --ignore-revs-file=.git-blame-ignore-revs` si no está configurado globalmente.
