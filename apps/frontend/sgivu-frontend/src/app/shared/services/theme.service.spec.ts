...
import { ThemeService, ThemePreference, Theme } from './theme.service';
...
describe('ThemeService', () => {
  ...
  describe('initialize()', () => {
    ...
    it('debe emitir cambios en preferenceSubject al inicializar', (done) => {
      localStorage.setItem('sgivu-theme', 'light');
      service['initialized'] = false;

      let emittedPreference: ThemePreference | undefined;
      service.preference$.subscribe((preference) => {
        emittedPreference = preference;
      });

      service.initialize();

      expect(emittedPreference).toBe('light');
      done();
    });

    it('debe emitir cambios en activeThemeSubject al inicializar', (done) => {
      localStorage.setItem('sgivu-theme', 'dark');
      service['initialized'] = false;
  ...
  describe('toggleTheme()', () => {
    ...
    it('debe persistir preferencia y emitir cambios en activeTheme$', (done) => {
      service.setPreference('light');
      let emittedTheme: Theme | undefined;
  ...
  describe('setPreference()', () => {
    ...
    it('debe persistir en localStorage y emitir en preferenceSubject', (done) => {
      let emittedPreference: ThemePreference | undefined;
      ...
      expect(emittedPreference).toBe('dark');
      expect(localStorage.setItem).toHaveBeenCalledWith('sgivu-theme', 'dark');
      done();
    });
  });
  ...
  describe('localStorage & BehaviorSubject', () => {
    ...
    it('debe emitir valores correctos en BehaviorSubjects', (done) => {
      const preferences: ThemePreference[] = [];
      const themes: Theme[] = [];

      service.preference$.subscribe((p) => preferences.push(p));
      service.activeTheme$.subscribe((t) => themes.push(t));
