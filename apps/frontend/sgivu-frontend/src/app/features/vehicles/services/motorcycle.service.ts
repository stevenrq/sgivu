...
export class MotorcycleService {
  ...
  private readonly motorcyclesState: WritableSignal<Motorcycle[]> = signal<
    Motorcycle[]
  >([]);

  private readonly motorcyclesPagerState: WritableSignal<
    PaginatedResponse<Motorcycle>
  > = signal<PaginatedResponse<Motorcycle>>(
    {} as PaginatedResponse<Motorcycle>,
  );
   ...
   */
  getAllPaginated(page: number): Observable<PaginatedResponse<Motorcycle>> {
    return this.http
  ...
  changeStatus(id: number, status: VehicleStatus): Observable<VehicleStatus> {
    return this.http
      .patch<{
        status: string;
      }>(`${this.apiUrl}/${id}/status`, JSON.stringify(status), { headers: this.jsonHeaders })
      .pipe(
        tap((response) => {
          this.motorcyclesState.update((motorcycles) =>
            motorcycles.map((motorcycle) =>
