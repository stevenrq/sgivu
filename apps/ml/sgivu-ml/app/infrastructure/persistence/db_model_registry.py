...
class DatabaseModelRegistry:
    ...
    async def save(self, model: Any, metadata: dict[str, Any]) -> ModelMetadata:
        ...
        El modelo se serializa con `joblib` a un `BYTEA` y se almacena junto
        con la metadata en la tabla `ml_model_artifacts` (ORM `ModelArtifact`).

        Parameters
        ----------
        model : Any
            Pipeline o estimador serializable por `joblib`.
        metadata : dict
            Diccionario con metadata del entrenamiento.

        Returns
        -------
        ModelMetadata
            Metadata extendida con el campo `version` generado.
        """
    ...
    async def load_latest(self) -> tuple[Any, ModelMetadata]:
        ...
        tuple[Any, ModelMetadata]
            Tupla `(model, metadata)` con el objeto deserializado y la
            metadata asociada.

        Raises
        ------
        FileNotFoundError
            Si no existe un artefacto almacenado.
        """
        async with self._session_factory() as session:
    ...
    async def latest_metadata(self) -> ModelMetadata | None:
        ...
        -------
        ModelMetadata | None
            Metadata del último artefacto si existe, o `None` en caso contrario.
        """
        async with self._session_factory() as session:
            result = await session.execute(
                select(ModelArtifact)
