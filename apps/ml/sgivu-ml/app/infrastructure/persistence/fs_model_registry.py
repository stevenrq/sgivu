...
class FileSystemModelRegistry:
    ...
    async def save(self, model: Any, metadata: dict[str, Any]) -> ModelMetadata:
        ...
        El artefacto se guarda con `joblib` en la ruta definida por
        `settings.model_path()` y se escribe un `latest.json` con la
        metadata del modelo (incluyendo el campo `version`).

        Parameters
        ----------
        model : Any
            Objeto serializable (pipeline de sklearn/xgboost) a persistir.
        metadata : dict
            Diccionario con metadata del entrenamiento (metrics, features, etc.).

        Returns
        -------
        ModelMetadata
            Metadata extendida con el campo `version` asignado.
        """
    ...
    async def load_latest(self) -> tuple[Any, ModelMetadata]:
        ...
        tuple[Any, ModelMetadata]
            Tupla `(model, metadata)` donde `model` es el pipeline cargado
            y `metadata` la metadata asociada.

        Raises
        ------
        FileNotFoundError
            Si no existe metadata previa (`latest.json`) o el artefacto.
        """
    ...
    async def latest_metadata(self) -> ModelMetadata | None:
        ...
        -------
        ModelMetadata | None
            Metadata si existe `latest.json`, o `None` si no hay modelo.
        """
