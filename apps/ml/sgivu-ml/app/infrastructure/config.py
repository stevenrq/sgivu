...
class Settings(BaseSettings):
    ...
    def database_dsn(self) -> str | None:
        """Genera el DSN de conexión con driver ``asyncpg``."""
        if self.database_url:
            dsn = self.database_url
            # Normaliza el driver a asyncpg para compatibilidad con SQLAlchemy async.
            for old_prefix in ("postgresql+psycopg2://", "postgresql://"):
                if dsn.startswith(old_prefix):
                    dsn = dsn.replace(old_prefix, "postgresql+asyncpg://", 1)
                    break
            return dsn
