...
Provee funciones de autenticación y autorización reutilizables
como ``Depends`` de FastAPI.
"""
...
from app.infrastructure.security.jwks_cache import (
    jwks_cache,
...
async def decode_token(raw_token: str) -> dict:
    """Decodifica y valida un JWT contra las llaves JWKS del issuer."""
    settings = get_settings()

    discovery_config = await oidc_discovery_cache.discover(settings)
...
def _extract_permissions(claims: dict) -> Set[str]:
    """Extrae permisos del claim ``rolesAndPermissions``."""
...
async def require_token(
    ...
    """Dependencia que exige un Bearer token válido."""
...
def require_permissions(required: Iterable[str]):
    """Fábrica de dependencia que exige al menos uno de los permisos indicados."""
...
def require_internal_or_permissions(required: Iterable[str]):
    """Fábrica de dependencia que acepta clave interna o token con permisos."""
    required_set = {permission for permission in required if permission}
    ...
    async def dependency(
        ...
        if internal_key:
            ...
            if settings.service_internal_secret_key and hmac.compare_digest(
                internal_key, settings.service_internal_secret_key
            ):
