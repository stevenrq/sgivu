...
public class SecurityConfig {
            ...
            authorizationServer ->
                authorizationServer
                    .authorizationEndpoint(
                        authorizationEndpoint -> authorizationEndpoint.consentPage(CONSENT_PATH))
    ...
    http.authorizeHttpRequests(
            authorize ->
                authorize
                    ...
                    .anyRequest()
                    .authenticated())
        .csrf(csrf -> csrf.ignoringRequestMatchers("/sso-logout"))
        .formLogin(
            formLogin ->
                formLogin
                    .loginPage(LOGIN_PATH)
                    .permitAll()
                    .defaultSuccessUrl(LOGIN_PATH)
  ...
  CorsConfigurationSource corsConfigurationSource() {
    ...
    // Cabeceras explícitas requeridas por el flujo OAuth2/OIDC y la SPA
    config.setAllowedHeaders(
        List.of("Authorization", "Content-Type", "Cache-Control", "Pragma", "X-Requested-With"));
    // Métodos explícitos; OPTIONS es necesario para el preflight CORS
    config.setAllowedMethods(List.of("GET", "POST", "OPTIONS"));
    // Orígenes permitidos: SPA Angular y Swagger UI servido por el gateway (valor según entorno)
    config.setAllowedOriginPatterns(
        List.of(angularClientProperties.getUrl(), gatewayClientProperties.getUrl()));
  ...
  OAuth2TokenCustomizer<JwtEncodingContext> jwtCustomizer(UserDetailsService userDetailsService) {
    return context -> {
      ...
      if (context.getTokenType().equals(OAuth2TokenType.ACCESS_TOKEN)) {
        ...
        // Red de seguridad: el flujo principal del gateway sincroniza el id_token del OidcUser
        // en cada refresh (ver OidcIdTokenSyncingRefreshTokenResponseClient), de modo que el
        // id_token_hint enviado al /connect/logout siempre coincide con el indexado en la
        // OAuth2Authorization del auth server. No obstante, si el gateway se reinicia justo
        // después de un refresh y la sincronización no llegó a persistirse en Redis, el
        // id_token del OidcUser podría estar ligeramente desfasado; mantener un TTL holgado
        // (≥ refreshTokenTimeToLive) garantiza que la validación JWT (exp) no falle en ese
        // margen y deja funcional la alternativa de fallback vía /sso-logout.
