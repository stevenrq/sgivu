...
class LoginFormHandler {
  ...
  async validateCredentials() {
    ...
    try {
      ...
      // TypeError = fallo de red (servidor caído). Redirigir a Angular en lugar de
      // dejar que el form POST falle con "Unable to connect".
      if (error instanceof TypeError) {
        const url = document.querySelector(
          'meta[name="angular-login-url"]',
        )?.content;
...
function startAuthHealthPolling() {
  const angularLoginUrl = document.querySelector(
    'meta[name="angular-login-url"]',
  )?.content;
  if (!angularLoginUrl) return;
