...
class ContractBusinessRuleServiceTest {
  ...
  class ApplyRulesTests {
    ...
    @Test
    @DisplayName("Venta lanza excepción cuando salePrice es inválido y no hay ventas previas")
    void shouldThrowWhenInvalidSalePriceAndNoPreviousSale() {
      PurchaseSaleRequest request = new PurchaseSaleRequest();
      ...
      assertEquals("No se encontró una compra válida asociada al vehículo.", ex.getMessage());
    }
  ...
  @Nested
  @DisplayName("findLatestSalePrice(List<PurchaseSale>, Double) — resuelve desde compras")
  class FindLatestSalePriceTests {

    @Test
    @DisplayName("Debe retornar último precio de venta desde compras ACTIVE/COMPLETED")
    void shouldReturnLatestSalePriceFromActiveOrCompleted() {
      PurchaseSale old = new PurchaseSale();
      old.setContractType(ContractType.PURCHASE);
      old.setContractStatus(ContractStatus.COMPLETED);
      old.setSalePrice(2500d);
      old.setUpdatedAt(LocalDateTime.now().minusDays(4));

      PurchaseSale recent = new PurchaseSale();
      recent.setContractType(ContractType.PURCHASE);
      recent.setContractStatus(ContractStatus.ACTIVE);
      recent.setSalePrice(2800d);
      recent.setUpdatedAt(LocalDateTime.now());

      PurchaseSale otherPending = new PurchaseSale();
      otherPending.setContractType(ContractType.PURCHASE);
      otherPending.setContractStatus(ContractStatus.PENDING);
      otherPending.setSalePrice(3500d);
      otherPending.setUpdatedAt(LocalDateTime.now().plusDays(1));

      Double result = service.findLatestSalePrice(List.of(old, recent, otherPending), null);

      assertEquals(2800d, result);
    }

    @Test
    @DisplayName("Debe retornar alternativo cuando no hay compras válidas y alternativo > 0")
    void shouldReturnFallbackWhenNoValidSales() {
      Double result = service.findLatestSalePrice(Collections.emptyList(), 1900d);

      assertEquals(1900d, result);
    }

    @Test
    @DisplayName(
        "Debe lanzar excepción cuando el contrato de compra existe pero su salePrice es nulo y el"
            + " fallback es inválido")
    void shouldThrowWhenPurchaseHasNoSalePriceAndFallbackInvalid() {
      PurchaseSale purchase = new PurchaseSale();
      purchase.setContractType(ContractType.PURCHASE);
      purchase.setContractStatus(ContractStatus.ACTIVE);
      purchase.setPurchasePrice(15000000d);
      purchase.setSalePrice(null); // contrato de compra sin salePrice

      PurchaseSaleRequest request = new PurchaseSaleRequest();
      request.setContractType(ContractType.SALE);
      request.setContractStatus(ContractStatus.PENDING);
      request.setSalePrice(0d); // fallback inválido
      request.setPurchasePrice(15000000d);

      assertThrows(
          ContractValidationException.class,
          () -> service.prepareSaleRequest(request, List.of(purchase)));
    }

    @Test
    @DisplayName("Debe lanzar excepción cuando no hay compras válidas y el alternativo es inválido")
    void shouldThrowWhenNoValidSalesAndNoFallback() {
      assertThrows(
          ContractValidationException.class,
          () -> service.findLatestSalePrice(Collections.emptyList(), null));
      assertThrows(
          ContractValidationException.class,
          () -> service.findLatestSalePrice(Collections.emptyList(), 0d));
    }
  }
