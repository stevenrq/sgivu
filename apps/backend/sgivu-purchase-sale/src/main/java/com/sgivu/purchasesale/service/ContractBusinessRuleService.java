...
public class ContractBusinessRuleService {
  ...
  public void prepareSaleRequest(
    ...
    if (salePrice == null || salePrice <= 0) {
      request.setSalePrice(findLatestSalePrice(contractsByVehicle, request.getSalePrice()));
    }
  ...
  /**
   * Busca el precio de venta más reciente entre los contratos de compra ACTIVE/COMPLETED. Se usa
   * {@code updatedAt} (no {@code createdAt}) porque un contrato puede renegociarse y actualizarse
   * tras su creación. Si no hay compras válidas, intenta el fallback proporcionado por el usuario.
   *
   * @param contractsByVehicle histórico de contratos del vehículo
   * @param fallbackSalePrice precio alternativo proporcionado directamente en el request
   * @return el precio de venta a usar en el contrato de venta
   * @implSpec El precio de venta del vehículo en un contrato de venta se determina por la compra
   *     más reciente. No tiene sentido tomar el precio de venta de otro contrato de venta porque el
   *     precio de venta de una venta no afecta el precio de venta de otra venta. Por eso solo se
   *     consideran contratos de compra para resolver el precio de venta. Si no hay contrato de
   *     compra válido para el vehículo, se intenta el fallback proporcionado por el usuario.
   */
  public Double findLatestSalePrice(
      List<PurchaseSale> contractsByVehicle, Double fallbackSalePrice) {
    return contractsByVehicle.stream()
        .filter(contract -> contract.getContractType() == ContractType.PURCHASE)
        .filter(
            contract ->
        ...
        .max(
            Comparator.comparing(
                PurchaseSale::getUpdatedAt, Comparator.nullsLast(Comparator.naturalOrder())))
        .map(PurchaseSale::getSalePrice)
        .orElseGet(
            () -> {
              if (fallbackSalePrice != null && fallbackSalePrice > 0) {
