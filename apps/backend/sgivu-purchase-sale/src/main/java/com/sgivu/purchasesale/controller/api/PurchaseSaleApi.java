...
public interface PurchaseSaleApi {
  ...
  @Operation(
      ...
      description =
          "Devuelve una lista de IDs de vehículos que han sido comprados pero aún no vendidos, lo"
              + " que los hace disponibles para nuevas ventas.")
  @ApiResponse(
      responseCode = "200",
      description = "Lista de IDs de vehículos disponibles",
      content = @Content(schema = @Schema(implementation = Long.class, type = "array")))
  @GetMapping(value = "/available-vehicles")
  ResponseEntity<List<Long>> getAvailableVehicleIds();

  @Operation(
      summary = "Snapshot agregado para el dashboard",
      description =
          "Devuelve en una sola llamada los conteos por estado, método de pago, actividad reciente,"
              + " agregados mensuales y conteos de vehículos. Diseñado para reemplazar el forkJoin"
              + " que hace el frontend en /dashboard. Respuesta cacheada ~60s en Redis.")
  @ApiResponse(
      responseCode = "200",
