...
public class PurchaseSaleReportService {
  ...
  private static final String LABEL_EN_REPARACION = "En reparación";
  ...
  private final Map<String, String> vehicleStatusLabels = new HashMap<>();
  ...
  public byte[] generatePdf(LocalDate startDate, LocalDate endDate) {
    ...
    try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
      ...
      throw new IllegalStateException("Error generating PDF report", ex);
    }
  }
  ...
  public byte[] generateExcel(LocalDate startDate, LocalDate endDate) {
    ...
    try (Workbook workbook = new XSSFWorkbook();
      ...
      throw new IllegalStateException("Error generating Excel report", ex);
    }
  }
  ...
  public byte[] generateCsv(LocalDate startDate, LocalDate endDate) {
    ...
    try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
      ...
      throw new IllegalStateException("Error generating CSV report", ex);
    }
  }
  ...
  private String formatVehicleBlock(VehicleSummary vehicleSummary) {
    if (vehicleSummary == null) {
      return "Vehículo: N/D" + '\n' + LABEL_ESTADO + getVehicleStatus(null);
    }
    ...
    return LABEL_TIPO
        ...
        + '\n'
        + LABEL_ESTADO
        + getVehicleStatus(vehicleSummary);
  }
  ...
  private String getVehicleStatus(VehicleSummary summary) {
    String rawStatus = summary == null ? null : summary.getStatus();
    if (rawStatus == null || rawStatus.isBlank()) {
      return UNKNOWN_STATUS;
    }

    String normalizedStatus = rawStatus.trim().toUpperCase(Locale.ROOT);
    if ("N/D".equals(normalizedStatus) || "ND".equals(normalizedStatus)) {
      return UNKNOWN_STATUS;
    }

    return vehicleStatusLabels.getOrDefault(normalizedStatus, normalizedStatus);
  }
  ...
  private void initialiseLabels() {
    ...
    vehicleStatusLabels.put("AVAILABLE", "Disponible");
    vehicleStatusLabels.put("SOLD", "Vendido");
    vehicleStatusLabels.put("IN_MAINTENANCE", "En mantenimiento");
    vehicleStatusLabels.put("IN_REPAIR", LABEL_EN_REPARACION);
    vehicleStatusLabels.put("IN_USE", "En uso");
    vehicleStatusLabels.put("INACTIVE", "Inactivo");
    vehicleStatusLabels.put("DISPONIBLE", "Disponible");
    vehicleStatusLabels.put("VENDIDO", "Vendido");
    vehicleStatusLabels.put("EN MANTENIMIENTO", "En mantenimiento");
    vehicleStatusLabels.put("EN REPARACION", LABEL_EN_REPARACION);
    vehicleStatusLabels.put("EN REPARACIÓN", LABEL_EN_REPARACION);
    vehicleStatusLabels.put("EN USO", "En uso");
    vehicleStatusLabels.put("INACTIVO", "Inactivo");
    vehicleStatusLabels.put(UNKNOWN_STATUS, UNKNOWN_STATUS);
  }
