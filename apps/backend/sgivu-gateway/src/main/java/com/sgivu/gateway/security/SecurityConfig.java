...
public class SecurityConfig {
  ...
  @Value("${spring.security.oauth2.client.registration.sgivu-gateway.client-secret}")
  ...
  /**
   * Construye el repositorio de registros OAuth2 con endpoints explícitos para evitar que Spring
   * Boot haga OIDC discovery (HTTP al auth server) durante la inicialización del contexto. Sin
   * esto, el gateway no puede arrancar si {@code sgivu-auth} está caído.
   *
   * <p>Incluye {@code end_session_endpoint} en el metadata del proveedor para que {@link
   * OidcClientInitiatedServerLogoutSuccessHandler} pueda hacer RP-Initiated Logout.
   */
  @Bean
  ReactiveClientRegistrationRepository clientRegistrationRepository() {
    String authUrl = servicesProperties.getMap().get("sgivu-auth").getUrl();
    ClientRegistration registration =
        ClientRegistration.withRegistrationId("sgivu-gateway")
            ...
            .tokenUri(authUrl + "/oauth2/token")
            .jwkSetUri(authUrl + "/oauth2/jwks")
            .userInfoUri(authUrl + "/userinfo")
            .userInfoAuthenticationMethod(AuthenticationMethod.HEADER)
            .userNameAttributeName(IdTokenClaimNames.SUB)
            .providerConfigurationMetadata(
                Map.of("end_session_endpoint", authUrl + "/connect/logout"))
            .build();
  ...
  ReactiveJwtDecoder jwtDecoder() {
    ...
    NimbusReactiveJwtDecoder decoder =
        NimbusReactiveJwtDecoder.withJwkSetUri(authUrl + "/oauth2/jwks").build();
    decoder.setJwtValidator(JwtValidators.createDefaultWithIssuer(authUrl));
    return decoder;
  }
   ...
   *       post_logout_redirect_uri} hacia Angular. Este es el flujo estándar y el camino normal.
   *   <li><strong>Sin OidcUser (sesión expirada)</strong>: redirige a {@code /sso-logout} del auth
   *       server como fallback. Sin sesión no hay {@code id_token_hint} y el endpoint OIDC
   *       fallaría; el fallback invalida la {@code HttpSession} JDBC del auth server y evita
   *       re-autenticación silenciosa.
   * </ul>
   *
   * <p><strong>Sincronización del {@code id_token} durante refresh</strong>: Spring Authorization
   * Server genera un {@code id_token} nuevo cada vez que el gateway refresca el access token (scope
   * {@code openid}) y reemplaza el anterior en su {@code OAuth2Authorization}. Spring Security no
   * actualiza el {@code OidcUser} de la sesión durante un refresh, por lo que el {@code
   * id_token_hint} quedaría obsoleto y el endpoint OIDC respondería {@code invalid_token}. Para
   * mantener el cumplimiento de OpenID Connect Session Management 1.0, el gateway usa un {@link
   * OidcIdTokenSyncingRefreshTokenResponseClient} que actualiza el {@code OidcUser} tras cada
   * refresh exitoso — ver {@link #authorizedClientManager}.
   */
  @Bean
  ServerLogoutSuccessHandler logoutSuccessHandler(
    ...
    log.info(
        "Logout success handler configured — OIDC RP-Initiated Logout to Angular /login, "
            + "fallback SSO logout: {}",
  ...
  ReactiveOAuth2AuthorizedClientManager authorizedClientManager(
    ...
    // Custom access token response client: tras cada refresh exitoso, sincroniza el id_token del
    // OidcUser en la WebSession para que el RP-Initiated Logout (OIDC) siga funcionando.
    // Ver OidcIdTokenSyncingRefreshTokenResponseClient para el detalle.
    refreshTokenProvider.setAccessTokenResponseClient(
    ...
    return authorizeRequest -> {
      // Propagamos el ServerWebExchange al Reactor Context para que el
      // OidcIdTokenSyncingRefreshTokenResponseClient pueda persistir el SecurityContext
      // actualizado vía ServerSecurityContextRepository.save(exchange, context).
      ServerWebExchange exchange = authorizeRequest.getAttribute(ServerWebExchange.class.getName());
      Mono<org.springframework.security.oauth2.client.OAuth2AuthorizedClient> authorizeChain =
              ...
              .onErrorResume(
                  ...
                  ex -> {
                    ...
                    if (OAuth2ErrorCodes.INVALID_GRANT.equals(errorCode)) {
                      ...
                      // Se invalida la sesión Redis y se retorna Mono.empty() para que el flujo
                      // termine en 401 → re-autenticación
                      log.warn(
                          "OAuth2 token refresh failed — invalid_grant [registration={}, "
                              + "description={}]. Invalidating session to force re-authentication.",
                      ...
                      if (exchange != null) {
                        return exchange
                            ...
                            .doOnNext(
                                session ->
                                    log.info(
                                        "Invalidating orphaned Redis session after "
                                            + "invalid_grant [id={}]",
                                        session.getId()))
                            .flatMap(WebSession::invalidate)
                            .then(Mono.empty());
                      ...
                      log.warn(
                          "No ServerWebExchange available in authorize request "
                              + "attributes — cannot invalidate session");
                      return Mono.empty();
                    }
      ...
      if (exchange != null) {
        return authorizeChain.contextWrite(ctx -> ctx.put(ServerWebExchange.class, exchange));
      }
      return authorizeChain;
    };
  }
