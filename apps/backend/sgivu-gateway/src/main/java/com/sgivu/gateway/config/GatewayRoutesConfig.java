...
public class GatewayRoutesConfig {
  ...
  @Value("${SGIVU_ML_URL:http://sgivu-ml:8000}")
  ...
  RouteLocator customRouteLocator(RouteLocatorBuilder builder) {
    ...
    return builder
        ...
        // ML (Aprendizaje automático) — retrain tiene timeout propio (1800 s / 30 min)
        .route(
            "sgivu-ml-retrain",
            r ->
                r.path("/v1/ml/retrain")
                    .filters(
                        f ->
                            f.tokenRelay()
                                .circuitBreaker(
                                    c ->
                                        c.setName("mlRetrainCircuitBreaker")
                                            .setFallbackUri("forward:/fallback/ml-retrain")))
                    .uri(mlServiceUrl))
        .route(
            "sgivu-ml",
            r ->
