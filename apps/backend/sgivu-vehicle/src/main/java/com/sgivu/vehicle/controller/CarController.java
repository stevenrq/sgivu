...
public class CarController implements CarApi {
  ...
  public ResponseEntity<Map<String, Long>> getCarCounts() {
    // count() + countByStatus() ejecutan SELECT COUNT(*) en BD.
    // Antes se hacía findAll().size() -> traía todos los registros a memoria.
  ...
  @Override
  @PreAuthorize("hasAuthority('car:read')")
  public ResponseEntity<List<CarResponse>> getByIds(List<Long> ids) {
    if (ids == null || ids.isEmpty()) {
      return ResponseEntity.ok(List.of());
    }
    List<CarResponse> responses =
        carService.findAllByIds(ids).stream().map(vehicleMapper::toCarResponse).toList();
