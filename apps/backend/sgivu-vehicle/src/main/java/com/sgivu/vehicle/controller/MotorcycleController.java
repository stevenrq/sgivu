...
public class MotorcycleController implements MotorcycleApi {
  ...
  public ResponseEntity<Map<String, Long>> getMotorcycleCounts() {
    long totalMotorcycles = motorcycleService.count();
  ...
  @Override
  @PreAuthorize("hasAuthority('motorcycle:read')")
  public ResponseEntity<List<MotorcycleResponse>> getByIds(List<Long> ids) {
    if (ids == null || ids.isEmpty()) {
      return ResponseEntity.ok(List.of());
    }
    List<MotorcycleResponse> responses =
        motorcycleService.findAllByIds(ids).stream()
            .map(vehicleMapper::toMotorcycleResponse)
            .toList();
