...
public class SecurityConfig {
  ...
  JwtDecoder jwtDecoder() {
    String issuerUrl = servicesProperties.getMap().get("sgivu-auth").getUrl();
    NimbusJwtDecoder decoder = NimbusJwtDecoder.withJwkSetUri(issuerUrl + "/oauth2/jwks").build();
    decoder.setJwtValidator(JwtValidators.createDefaultWithIssuer(issuerUrl));
    return decoder;
  }
